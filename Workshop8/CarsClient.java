...
public class CarsClient {
	public static void main(String[] args) {
		...
		try {
			...
			String s;
			try(BufferedReader br = new BufferedReader(new FileReader(path + "/src/" + "car.txt"))){
				while((s = br.readLine()) != null){
					String[] tok = s.split(",");
						cars.add(new Car(tok[0], tok[1], Double.parseDouble(tok[2])));
				}
			} catch(Exception e){
			...
			for (int i = 0; i < 3; i++) {
				try {
					...
					oosToServer.writeObject(cars.get(i));
					oosToServer.flush();
					System.out.println("send car to the server for registration:" + cars.get(i));

					// receive an object from the server
					cars.set(i, (Car) oisFromServer.readObject()); // casting!
					System.out.println("the car returned by the server:" + cars.get(i));
